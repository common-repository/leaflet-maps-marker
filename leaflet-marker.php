...
$layer_show_button = ($addtoLayer != NULL && $addtoLayer != 0) ? "<a class='button-secondary lmm-nav-secondary' href=" . LEAFLET_WP_ADMIN_URL . "admin.php?page=leafletmapsmarker_layer&id=" . $addtoLayer .">" . __('edit assigned layer','leaflet-maps-marker') . "</a>&nbsp;&nbsp;&nbsp;" : "";
...
if (!empty($action)) {
$markernonce = isset($_POST['_wpnonce']) ? esc_attr__($_POST['_wpnonce']) : (isset($_GET['_wpnonce']) ? esc_attr__($_GET['_wpnonce']) : '');
if (! wp_verify_nonce($markernonce, 'marker-nonce') ) die('<br/>'.__('Security check failed - please call this function from the according admin page!','leaflet-maps-marker').'');
  ...
  elseif ($action == 'delete') {
	...
	$wpdb->query( $result );
	$wpdb->query( "OPTIMIZE TABLE `$table_name_markers`" );
        echo '<p><div class="updated" style="padding:10px;">' . __('Marker has been successfully deleted','leaflet-maps-marker') . '</div><a class=\'button-secondary lmm-nav-secondary\' href=\'' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_markers\'>' . __('list all markers','leaflet-maps-marker') . '</a>&nbsp;&nbsp;&nbsp;<a class=\'button-secondary lmm-nav-secondary\' href=\'' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker\'>' . __('add new marker','leaflet-maps-marker') . '</a></p>';
    }
  }
...
if ($lat === NULL) {
$error_marker_not_exists = sprintf( esc_attr__('Error: a marker with the ID %1$s does not exist!','leaflet-maps-marker'), htmlspecialchars($_GET['id']));
...
if ( $edit_status == 'updated') {
	echo '<p><div class="updated" style="padding:10px;">' . __('Marker has been successfully updated','leaflet-maps-marker') . '</div>';
} else if ( $edit_status == 'published') {
	echo '<p><div class="updated" style="padding:10px;">' . __('Marker has been successfully published','leaflet-maps-marker') . '</div>';
	...
	<form id="marker-add-edit" method="post">
		...
		?>
        <h1 style="margin:10px 0 10px 0;"><?php ($isedit === true) ? _e('Edit marker','leaflet-maps-marker') : _e('Add new marker','leaflet-maps-marker') ?>
		<?php echo ($isedit === true) ? ' "' . stripslashes($markername) . '" (ID '.$id.')' : '' ?>
		<input id="submit_top" disabled style="font-weight:bold;margin-left:10px;" type="submit" name="marker" class="submit button-primary" value="<?php ($isedit === true) ? _e('update','leaflet-maps-marker') : _e('publish','leaflet-maps-marker') ?>" />
		</h1>
		<p style="margin-top:0px;">
		<?php echo sprintf(__('Add a new marker to display a single location. To display multiple locations at the same time, <a href="%1$s">create a layer</a> first (e.g. "company locations") and then add markers (e.g. "headquarters", "store A", "store B") assigned to that layer.','leaflet-maps-marker'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_layer'); ?>
		</p>
		<table class="widefat">
			<?php if ($isedit === true) { ?>
			<tr>
				<td style="width:230px;" class="lmm-border"><label for="shortcode"><strong><?php _e('Shortcode and API links','leaflet-maps-marker') ?></strong></label></td>
				...
				?>
				<br><small><?php _e('Use this shortcode in posts or pages on your website or one of the API links for embedding in external websites or apps','leaflet-maps-marker') ?></small>
					</td>
			</tr>
			<?php } ?>
            ...
            <tr>
				<td style="width:230px;" class="lmm-border"><label for="markername"><strong><?php _e('Marker name','leaflet-maps-marker') ?></strong></label></td>
			...
			<tr>
				...
				<td class="lmm-border">
					...
					if ($address == NULL) {
						echo '<span id="popup-address" style="display:none;">' . esc_attr__('if set, address will be displayed here','leaflet-maps-marker') . '</span>';
					} else {
					...
					<?php _e('latitude','leaflet-maps-marker') ?>: <input style="width: 100px;" type="text" id="lat" name="lat" value="<?php echo $lat; ?>" />
					<?php _e('longitude','leaflet-maps-marker') ?>: <input style="width: 100px;" type="text" id="lon" name="lon" value="<?php echo $lon; ?>" />
					</div>
				</td>
			</tr>
			<tr>
				<td class="lmm-border"><p style="margin-bottom:0px;"><strong><?php _e('Map size','leaflet-maps-marker') ?></strong><br/>
					...
					<label for="layer"><strong><?php _e('Layer','leaflet-maps-marker') ?></strong></label>
					<small><?php echo '<br/><a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade">' . __('Upgrade to pro for assigning markers to multiple layers','leaflet-maps-marker') . '</a><br/>' ?></small>
					<select id="layer" name="layer">
						<option value="0">
						<?php _e('not assigned to a layer','leaflet-maps-marker') ?>
						</option>
						<?php
						foreach ($layerlist as $row) {
							...
							if ($row['multi_layer_map'] == 0) {
								...
								echo '<option title="' . stripslashes(htmlspecialchars($row['name'])) . ' (' . esc_attr__('This is a multi-layer map - markers cannot be assigned to this layer directly','leaflet-maps-marker') . ')" value="' . $row['id'] . '"' . ( (($row['id'] == $layer) || ($row['id'] == $addtoLayer)) ? ' selected="selected"' : '') . ' disabled="disabled">' . mb_substr(stripslashes(htmlspecialchars($row['name'])), 0, 25) . $layername_abstract . ' (ID ' . $row['id'] . '/MLM)</option>';
							}
						}
						?>
					</select>
					<br>
					<small><?php echo $layereditlink = ($layer != 0) ? "<a href=\"" . LEAFLET_WP_ADMIN_URL . "admin.php?page=leafletmapsmarker_layer&id=".$layer."\">" . __('edit layer','leaflet-maps-marker') . " (ID ".$layer.")</a> " . __('or','leaflet-maps-marker') . "" : "" ?> <a tabindex="121" href="<?php LEAFLET_WP_ADMIN_URL ?>admin.php?page=leafletmapsmarker_layer">
					<?php _e('add new layer','leaflet-maps-marker') ?>
					</a>
					<br/>
					<a href="<?php echo LEAFLET_WP_ADMIN_URL; ?>admin.php?page=leafletmapsmarker_pro_upgrade" title="<?php echo esc_attr__(' Feature available in pro version only','leaflet-maps-marker'); ?>"><img src="<?php echo LEAFLET_PLUGIN_URL; ?>inc/img/icon-eye-show.png" width="16" height="16" alt="preview icon" style="float:left;margin:2px 3px 0 0;" /> <?php _e('preview all markers from assigned layer(s)','leaflet-maps-marker'); ?></a>
					</small>
					...
					<p>
					<hr style="border:none;color:#edecec;background:#edecec;height:1px;">
					<strong><?php _e('Controlbox for basemaps/overlays','leaflet-maps-marker') ?>:</strong></label><br/>
					<input style="margin-top:1px;" id="controlbox_hidden" type="radio" name="controlbox" value="0" <?php checked($controlbox, 0); ?>><label for="controlbox_hidden"><?php _e('hidden','leaflet-maps-marker') ?></label><br/>
					<input style="margin-top:1px;" id="controlbox_collapsed" type="radio" name="controlbox" value="1" <?php checked($controlbox, 1); ?>><label for="controlbox_collapsed"><?php _e('collapsed','leaflet-maps-marker') ?></label><br/>
					<input style="margin-top:1px;" id="controlbox_expanded" type="radio" name="controlbox" value="2" <?php checked($controlbox, 2); ?>><label for="controlbox_expanded"><?php _e('expanded','leaflet-maps-marker') ?></label>
				...
				<td style="padding-bottom:5px;" class="lmm-border">
					...
					echo '<div class="lmm-panel-api">';
						if ( (isset($lmm_options[ 'defaults_marker_panel_directions' ] ) == TRUE ) && ( $lmm_options[ 'defaults_marker_panel_directions' ] == 1 ) ) {
								...
								if ($lmm_options['directions_provider'] == 'googlemaps') {
									...
									echo '<a tabindex="105" href="https://www.bing.com/maps/default.aspx?v=2&rtp=pos___e_~pos.' . $lat . '_' . $lon . $bing_to . '" target="_blank" title="' . esc_attr__('Get directions','leaflet-maps-marker') . '"><img src="' . LEAFLET_PLUGIN_URL . 'inc/img/icon-car.png" width="14" height="14" class="lmm-panel-api-images" /></a>';
								}
						}
					echo '</div>'.PHP_EOL;
					echo '<div id="lmm-panel-text" class="lmm-panel-text" style="' . htmlspecialchars(addslashes($lmm_options[ 'defaults_marker_panel_paneltext_css' ])) . '">' . (($markername == NULL) ? __('if set, markername will be displayed here','leaflet-maps-marker') : stripslashes($markername)) . '</div>'.PHP_EOL;
					?>
					</div> <!--end lmm-panel-->
					...
					if ($lmm_options['directions_popuptext_panel'] == 'yes') {
						...
						$directions_settings_link = ( (current_user_can("activate_plugins")) && ($current_editor == "advanced") ) ? " (<a tabindex='103' href='" . LEAFLET_WP_ADMIN_URL . "admin.php?page=leafletmapsmarker_settings#directions' title='" . esc_attr__("change directions settings","leaflet-maps-marker") . "'>" . __("Settings","leaflet-maps-marker") . "</a>)" : "";
					} else {
			...
			<tr>
				<td class="lmm-border"><label for="default_icon"><strong><?php _e('Icon', 'leaflet-maps-marker') ?></strong></label>
					<br/>
					<?php
					if ($current_editor == 'simplified') {
					...
					<small>
					<?php
					$mapicons_admin = sprintf( __('If you want to use different icons, please visit the %1$s (offering more than 700 compatible icons) and upload the new icons to the directory %2$s/','leaflet-maps-marker'), '<a tabindex="112" href="https://mapicons.mapsmarker.com" target="_blank">Map Icons Collection</a>', LEAFLET_PLUGIN_ICONS_URL);
					$mapicons_user = sprintf( __('If you want to use different icons, please visit the %1$s (offering more than 700 compatible icons) and ask your WordPress admin to upload the new icons to the directory %2$s/','leaflet-maps-marker'), '<a tabindex="113" href="https://mapicons.mapsmarker.com" target="_blank">Map Icons Collection</a>', LEAFLET_PLUGIN_ICONS_URL);
					$upload_icons_button_text = '<br/><br/>' . __('You can also upload the icons by clicking the button "upload new icon"','leaflet-maps-marker') . ' <a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade" title="' . esc_attr__('This feature is available in the pro version only! Click here to find out how you can start a free 30-day-trial easily','leaflet-maps-marker') . '">(' . __('pro version only','leaflet-maps-marker') . ')';
					...
					if (is_multisite()) {
					$mapicons_directory = sprintf( __('As you are running your blog within a WordPress Multisite installation, please use this icon directory on your server: %2$s/','leaflet-maps-marker'), '<a tabindex="114" href="https://mapicons.mapsmarker.com" target="_blank">Map Icons Collection</a>', LEAFLET_PLUGIN_ICONS_DIR);
				...
				<td class="lmm-border"><?php
					if ($current_editor == 'simplified') {
						...
						echo '<div style="text-align:center;float:left;line-height:0px;padding:17px 5px 0 5px;" id="showlessicons"><a style="cursor:pointer;">' . __('show fewer icons','leaflet-maps-marker') . '</a></div>';
						echo '<a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade" title="' . esc_attr__('This feature is available in the pro version only! Click here to find out how you can start a free 30-day-trial easily','leaflet-maps-marker') . '"><img style="margin:3px 0 0 7px;" src="' . LEAFLET_PLUGIN_URL . 'inc/img/pro-feature-filter-icons.png"></a>';
						echo '<a href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_pro_upgrade" title="' . esc_attr__('This feature is available in the pro version only! Click here to find out how you can start a free 30-day-trial easily','leaflet-maps-marker') . '"><img style="margin:3px 0 0 7px;" src="' . LEAFLET_PLUGIN_URL . 'inc/img/pro-feature-upload-icons.png"></a>';
						echo '</div>';
			...
			<tr>
				<td class="lmm-border"><p><label for="popuptext"><strong><?php _e('Popup text','leaflet-maps-marker') ?></strong></label>
				<br /><br />
				<label for="openpopup"><?php _e('open popup','leaflet-maps-marker') ?></label>&nbsp;&nbsp;<input type="checkbox" name="openpopup" id="openpopup" <?php checked($openpopup, 1 ); ?>>
				<br/><small>
				<?php _e('If unchecked, the popup will only be visible after clicking on the marker on marker- or layer-maps.','leaflet-maps-marker') ?>
				</small>
				<br /><br />
				<label for="add-markername"><?php _e('add markername','leaflet-maps-marker') ?></label>&nbsp;&nbsp;<input type="checkbox" name="add-markername" id="add-markername" disabled="disabled"><br/>
				<small><a href="<?php echo LEAFLET_WP_ADMIN_URL; ?>admin.php?page=leafletmapsmarker_pro_upgrade" title="<?php esc_attr_e('This feature is available in the pro version only! Click here to find out how you can start a free 30-day-trial easily','leaflet-maps-marker'); ?>"><?php _e('Feature available in pro version only','leaflet-maps-marker'); ?></a></small>
				<br /><br />
				<label for="add-directions"><?php _e('add directions link','leaflet-maps-marker') ?></label>&nbsp;&nbsp;<?php if ($lmm_options['directions_popuptext_panel'] == 'yes') { echo '<input type="checkbox" name="add-directions" id="add-directions" checked="checked" disabled="disabled">'; } else { echo '<input type="checkbox" name="add-directions" id="add-directions" disabled="disabled">'; } ?>
				<br/><small><a tabindex="119" href="<?php echo LEAFLET_WP_ADMIN_URL; ?>admin.php?page=leafletmapsmarker_settings#lmm-directions"><?php _e('Please visit Settings to change this globally','leaflet-maps-marker'); ?></a></small>
				</p>
				</td>
				<td class="lmm-border">
						...
						'tinymce' => array(
							...
							'setup' => 'function(ed) {
											ed.on("keyup", function(ed,e) {
												var popup_panel = "<div style=\"border-top:1px solid #f0f0e7;padding-top:5px;margin-top:5px;clear:both;\">"+document.getElementById("address").value+" <a href=\"' . $directionslink . '\" target=\"_blank\" title=\"' . esc_attr__('Get directions','leaflet-maps-marker') . '\">(' . __('Directions','leaflet-maps-marker') . ')</a>' . $directions_settings_link . '</div>";
				...
				<span style="display:none;" id="selectlayer-popuptext-hidden"><?php echo do_shortcode($popuptext_sanitized); ?></span>
                 (<a target="_blank" href="https://www.mapsmarker.com/multilingual" target="_blank"><?php _e('translate', 'leaflet-maps-marker'); ?></a> <img src="<?php echo LEAFLET_PLUGIN_URL; ?>inc/img/pro-feature-banner-small.png" width="68" height="9" border="0">)<br/>
				<small>
					<?php
					echo '<span style="' . $current_editor_css . '">' . sprintf( esc_attr__('Note: if you add an image, the following CSS definition will be applied: %1$s','leaflet-maps-marker'), '<code>' . htmlspecialchars($lmm_options['defaults_marker_popups_image_css']) . '</code>');
					if (current_user_can('activate_plugins')) { echo ' <a tabindex="102" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#lmm-mapdefaults-marker_popups" title="' . esc_attr__('can be changed at section "Default values for marker popups"','leaflet-maps-marker') . '">(' . __('Settings','leaflet-maps-marker') . ')</a>'; }
					?>
					</span>
					</small>
				</td>
			...
			<tr id="advanced-settings">
				...
				<td class="lmm-border">
				<div style="<?php echo $current_editor_css; ?>">
					<p><strong><?php _e('WMS layers','leaflet-maps-marker') ?></strong> <?php if (current_user_can('activate_plugins')) { echo '<a tabindex="101" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#wms">(' . __('Settings','leaflet-maps-marker') . ')</a>'; } ?></p>
					<?php
						...
						if ($j.isFunction($j(document).datetimepicker)) {
							$j(function() {
								$j("#kml_timestamp").datetimepicker({
									...
									changeMonth: true,
									changeYear: true,
									timeText: '<?php esc_attr_e('Time','leaflet-maps-marker'); ?>',
									hourText: '<?php esc_attr_e('Hour','leaflet-maps-marker'); ?>',
									minuteText: '<?php esc_attr_e('Minute','leaflet-maps-marker'); ?>',
									secondText: '<?php esc_attr_e('Second','leaflet-maps-marker'); ?>',
									currentText: '<?php esc_attr_e('Now','leaflet-maps-marker'); ?>',
									closeText: '<?php esc_attr_e('Add','leaflet-maps-marker'); ?>',
									timeFormat: 'hh:mm:ss',
									showSecond: true,
								});
							});
					...
					<label for="kml_timestamp"><strong><?php _e('Timestamp for KML animation','leaflet-maps-marker') ?>:</strong></label> <br/>
					<input type="<?php echo $datetime_ios_workaround; ?>" id="kml_timestamp" name="kml_timestamp" value="<?php echo $kml_timestamp ; ?>" /><br/>
					<small><?php _e('If empty, marker creation date will be used','leaflet-maps-marker') ?></small>
					<hr style="border:none;color:#edecec;background:#edecec;height:1px;">
					</div>
			...
			<tr style="<?php echo $current_editor_css; ?>">
				...
				<td><small>
					<?php
					echo __('Marker added by','leaflet-maps-marker') . ' ';
	...
	<table><tr><td>
	<input id="submit_bottom" disabled style="font-weight:bold;<?php echo $margin_top = ($isedit === false) ? 'margin-top:17px;' : '' ?>" type="submit" name="marker" class="submit button-primary" value="<?php ($isedit === true) ? _e('update','leaflet-maps-marker') : _e('publish','leaflet-maps-marker') ?>" />
	</form>
	</td>
	<td>
		<?php
				...
				echo '<input title="' . esc_attr__('Feature available in pro version only','leaflet-maps-marker') . '" class="submit button-secondary lmm-nav-secondary" type="submit" name="marker" value="' . __('duplicate', 'leaflet-maps-marker') . '" disabled="disabled" />';
				echo '</div></form>';
		?>
	</td>
	...
	<td>
		<form method="post">
			...
			<input type="hidden" name="action" value="delete" />
				<?php $confirm = sprintf( esc_attr__('Do you really want to delete marker %1$s (ID %2$s)?','leaflet-maps-marker'), $markername, $id) ?>
				<div class="submit" style="margin:0 0 0 40px;">
				<input id="delete" class="submit button-secondary lmm-nav-secondary" style="color:#FF0000;" type="submit" name="marker" value="<?php _e('delete', 'leaflet-maps-marker') ?>" onclick="return confirm('<?php echo $confirm ?>')" disabled="disabled" />
				</div>
		</form>
	</td>
	<?php } ?>
...
(function($) {
  selectlayer = new L.Map("selectlayer", { dragging: <?php echo $lmm_options['misc_map_dragging'] ?>, touchZoom: <?php echo $lmm_options['misc_map_touchzoom'] ?>, scrollWheelZoom: <?php echo $lmm_options['misc_map_scrollwheelzoom'] ?>, doubleClickZoom: <?php echo $lmm_options['misc_map_doubleclickzoom'] ?>, boxzoom: <?php echo $lmm_options['map_interaction_options_boxzoom'] ?>, trackResize: <?php echo $lmm_options['misc_map_trackresize'] ?>, worldCopyJump: <?php echo $lmm_options['map_interaction_options_worldcopyjump'] ?>, closePopupOnClick: <?php echo $lmm_options['misc_map_closepopuponclick'] ?>, keyboard: <?php echo $lmm_options['map_keyboard_navigation_options_keyboard'] ?>, keyboardPanOffset: <?php echo intval($lmm_options['map_keyboard_navigation_options_keyboardpanoffset']) ?>, keyboardZoomOffset: <?php echo intval($lmm_options['map_keyboard_navigation_options_keyboardzoomoffset']) ?>, inertia: <?php echo $lmm_options['map_panning_inertia_options_inertia'] ?>, inertiaDeceleration: <?php echo intval($lmm_options['map_panning_inertia_options_inertiadeceleration']) ?>, inertiaMaxSpeed: <?php echo intval($lmm_options['map_panning_inertia_options_inertiamaxspeed']) ?>, zoomControl: <?php echo $lmm_options['misc_map_zoomcontrol'] ?>, crs: <?php echo $lmm_options['misc_projections'] ?> });
	<?php
		...
		$osm_editlink = ($lmm_options['misc_map_osm_editlink'] == 'show') ? '&nbsp;(<a href=\"https://www.openstreetmap.org/edit?editor=' . $lmm_options['misc_map_osm_editlink_editor'] . '&amp;lat=' . $lat . '&amp;lon=' . $lon . '&zoom=' . $zoom . '\" target=\"_blank\" title=\"' . esc_attr__('help OpenStreetMap.org to improve map details','leaflet-maps-marker') . '\">' . __('edit','leaflet-maps-marker') . '</a>)' : '';
 ...
 if ($lmm_options['directions_popuptext_panel'] == 'yes') {
	 $directions_settings_link = ( (current_user_can('activate_plugins')) && ($current_editor == 'advanced') ) ? ' (<a tabindex="103" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_settings#lmm-directions" title="' . esc_attr__('change directions settings','leaflet-maps-marker') . '">' . __('Settings','leaflet-maps-marker') . '</a>)' : '';
	 if ($address == NULL) {
		$google_from = $lat . ',' . $lon;
		$address = esc_attr__('if set, address will be displayed here','leaflet-maps-marker');
	} else {
		$google_from = urlencode($address);
	}
	 $address = (($address == NULL) ? esc_attr__('if set, address will be displayed here','leaflet-maps-marker') : $address);
	 ...
	 if ($lmm_options['directions_provider'] == 'googlemaps') {
		...
		$popuptext = $popuptext . '(<a href="https://' . $gmaps_base_domain_directions . '/maps?daddr=' . $google_from . '&t=' . $lmm_options[ 'directions_googlemaps_map_type' ] . '&layer=' . $lmm_options[ 'directions_googlemaps_traffic' ] . '&doflg=' . $lmm_options[ 'directions_googlemaps_distance_units' ] . $avoidhighways . $avoidtolls . $publictransport . $walking . $google_language . '&om=' . $lmm_options[ 'directions_googlemaps_overview_map' ] . '" target="_blank" title="' . esc_attr__('Get directions','leaflet-maps-marker') . '">' . __('Directions','leaflet-maps-marker') . '</a>)';
	 } else if ($lmm_options['directions_provider'] == 'yours') {
		 $popuptext = $popuptext . '(<a href="http://www.yournavigation.org/?tlat=' . $lat . '&tlon=' . $lon . '&v=' . $lmm_options[ 'directions_yours_type_of_transport' ] . '&fast=' . $lmm_options[ 'directions_yours_route_type' ] . '&layer=' . $lmm_options[ 'directions_yours_layer' ] . '" target="_blank" title="' . esc_attr__('Get directions','leaflet-maps-marker') . '">' . __('Directions','leaflet-maps-marker') . '</a>';
	 } else if ($lmm_options['directions_provider'] == 'ors') {
		 $popuptext = $popuptext . '(<a href="http://www.openrouteservice.org/?pos=' . $lon . ',' . $lat . '&wp=' . $lon . ',' . $lat . '&zoom=' . $zoom . '&routeWeigh=' . $lmm_options[ 'directions_ors_routeWeigh' ] . '&routeOpt=' . $lmm_options[ 'directions_ors_routeOpt' ] . '&layer=' . $lmm_options[ 'directions_ors_layer' ] . '" target="_blank" title="' . esc_attr__('Get directions','leaflet-maps-marker') . '">' . __('Directions','leaflet-maps-marker') . '</a>)';
	 } else if ($lmm_options['directions_provider'] == 'bingmaps') {
		if ( $address != NULL ) { $bing_to = '_' . urlencode($address); } else { $bing_to = ''; }
		 $popuptext = $popuptext . '(<a href="https://www.bing.com/maps/default.aspx?v=2&rtp=pos___e_~pos.' . $lat . '_' . $lon . $bing_to . '" target="_blank" title="' . esc_attr__('Get directions','leaflet-maps-marker') . '">' . __('Directions','leaflet-maps-marker') . '</a>)';
	 }
