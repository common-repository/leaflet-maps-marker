...
$callback = (isset($_GET['callback']) == TRUE ) ? preg_replace( '/[^a-zA-Z0-9_]/', '', esc_attr__($_GET['callback'])) : '';
...
if (!lmm_is_plugin_active('leaflet-maps-marker/leaflet-maps-marker.php') ) {
	echo sprintf(__('The plugin "Leaflet Maps Marker" is inactive on this site and therefore this API link is not working.<br/><br/>Please contact the site owner (%1s) who can activate this plugin again.','leaflet-maps-marker'), antispambot(get_bloginfo('admin_email')) );
} else {
			...
			foreach ($markers as $marker) {
				...
				if ($lmm_options['directions_popuptext_panel'] == 'yes') {
					...
					if ($lmm_options['directions_provider'] == 'googlemaps') {
						...
						$mpopuptext = $mpopuptext . "<a href='http://www.bing.com/maps/default.aspx?v=2&rtp=pos___e_~pos." . $marker['mlat'] . "_" . $marker['mlon'] . $bing_to ."' target='_blank' title='" . esc_attr__('Get directions','leaflet-maps-marker') . "'>" . __('Directions','leaflet-maps-marker') . "</a>";
					}
