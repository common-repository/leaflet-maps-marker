...
function lmm_button_text($plugin_array) {
	...
	$text_add = __('Add Map','leaflet-maps-marker');
...
function get_mm_list(){
	...
	if (isset($_GET['q'])) {
		if (!wp_verify_nonce($get_map_search_nonce, 'map-search-nonce')) { die(__('Security check failed - please call this function from the according admin page!','leaflet-maps-marker').''); };
	}
	global $wpdb, $wp_version;
	$table_name_markers = $wpdb->prefix.'leafletmapsmarker_markers';
	$table_name_layers = $wpdb->prefix.'leafletmapsmarker_layers';

	$l_condition = isset($_GET['q']) ? "AND l.name LIKE '%" . esc_sql(sanitize_key($_GET['q'])) . "%'" : '';
	$m_condition = isset($_GET['q']) ? "AND m.markername LIKE '%" . esc_sql(sanitize_key($_GET['q'])) . "%'" : '';
	...
	<html>
	<head>
		<title><?php _e('Add Map','leaflet-maps-marker') ?></title>
	...
	<tr>
	<td style="width:42%;"><div id="msb_searchContainer" title="<?php echo sprintf(esc_attr__('If no search term is entered, the latest %1$s updated maps will be shown.','leaflet-maps-marker'), 50); ?>"><input type="text" name="q" id="msb_search" placeholder="<?php _e('Search','leaflet-maps-marker'); ?>"/></div></td>
	<?php 
		global $current_user;
		$lmm_options = get_option( 'leafletmapsmarker_options' );
		if (current_user_can( $lmm_options[ 'capabilities_edit' ])) {
			echo '<td><a class="newmap" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker" target="_blank" title="' . esc_attr__('create a new marker map','leaflet-maps-marker') . '"><img src="' . LEAFLET_PLUGIN_URL  . '/inc/img/icon-menu-add.png"> ' . __('new marker map', 'leaflet-maps-marker') . '</a></td>';
			echo '<td><a class="newmap" href="' . LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_layer" target="_blank" title="' . esc_attr__('create a new layer map','leaflet-maps-marker') . '"><img src="' . LEAFLET_PLUGIN_URL  . '/inc/img/icon-menu-add.png"> ' . __('new layer map', 'leaflet-maps-marker') . '</a></td>';
		}
	?>
	</tr>
	</table>
	<div id="msb_listContainer">
		<?php
		if ($marklist != NULL) {
			echo '<div id="msb_listHint">' . __('Please select the map you would like to add','leaflet-maps-marker') . ':</div>';
			buildMarkersList($marklist);
		} else {
			echo '<div id="no_map_yet">' . __('No map has been created yet.','leaflet-maps-marker') . '<br/>';
			if (current_user_can( $lmm_options[ 'capabilities_edit' ])) {
				echo sprintf(__('Please <a href="%s" target="_blank">click here</a> to add your first one!','leaflet-maps-marker'), LEAFLET_WP_ADMIN_URL . 'admin.php?page=leafletmapsmarker_marker');
			}
			echo '</div>';
		} ?>
	</div>
	<table style="width:100%;"><tr>
	<td style="padding-top:12px;"><a href="#" id="msb_cancel"><?php _e('Cancel','leaflet-maps-marker'); ?></a></td>
	<td>
		<span id="submit-enabled" style="display:none;"><input type="button" href="#" id="msb_insertMarkerSC" value="<?php esc_attr_e('Insert map','leaflet-maps-marker'); ?>" /></span>
		<span id="submit-disabled"><input class="disabled" type="button" href="#" id="msb_insertMarkerSC_disabled" value="<?php esc_attr_e('Insert map','leaflet-maps-marker'); ?>" disabled="disabled" title="<?php esc_attr_e('Please select a map first!','leaflet-maps-marker'); ?>" /></span>
		</td></tr>
	</table>
...
function buildMarkersList($array){
	foreach($array as $one):
		...
		if ($one['type'] == 'marker') {
			$title = __('Marker','leaflet-maps-marker'). ' ID ' . $one['id'];
			if ($one['icon'] == NULL) {
			...
			$title = __('Layer','leaflet-maps-marker'). ' ID ' . $one['id'];
	...
	<tr>
		<td style="width:30px;">
			<span class="name" title="<?php esc_attr_e('map type and ID','leaflet-maps-marker');?>"><?php echo $list_entry; ?></span>
		</td>
		<td valign="top" title="<?php esc_attr_e($name_title); ?>">
        		<span class="name"><?php echo $name; //sanitation done on lines 222ff ?></span>
		</td>
		<td valign="top" title="<?php esc_attr_e('updated on','leaflet-maps-marker');?> <?php echo $date; ?> <?php _e('by','leaflet-maps-marker'); ?> <?php echo $one['updatedby']?>">
			<span class="date"><?php echo $date; ?></span>
		</td>
			<td valign="top" style="padding:0 5px;width:10px;">
			<a class="preview" alt="<?php echo $one['type']; ?>" href="<?php echo LEAFLET_WP_ADMIN_URL; ?>admin.php?page=leafletmapsmarker_<?php echo $one['type']; ?>&id=<?php echo $one['id']; ?>" title="<?php esc_attr_e('open map in new window','leaflet-maps-marker'); ?>" target="_blank"><img src="<?php echo LEAFLET_PLUGIN_URL; ?>/inc/img/icon-menu-external.png"/></a>	
