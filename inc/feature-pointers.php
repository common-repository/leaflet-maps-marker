...
jQuery(document).ready(function($) {
	...
	<?php if ( !isset($dismissed_pointers["lmmesw"]) && ($page == 'leafletmapsmarker_marker' || $page == 'leafletmapsmarker_layer') ) { ?>
		$('#editmodeswitch').pointer({
			content: '<h3><?php esc_attr_e('You are using the simplified editor','leaflet-maps-marker'); ?></h3><p><?php esc_attr_e('If you want to be able to set all available options, please switch to the advanced editor.','leaflet-maps-marker'); ?></p>',
			position: {
				edge: 'right',
				align: 'center'
			},
